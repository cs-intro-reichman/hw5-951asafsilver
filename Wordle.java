public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        In dictReader = new In(filename); 
        String[] dictArray = dictReader.readAllStrings();
        dictReader.close();
        return dictArray;
    }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        ...
        for (int i = 0; i < secret.length(); i++) {
            if (guess.charAt(i) == secret.charAt(i)) {
                ...
                resultRow[i] = '_'; // שינוי ל-קו תחתון (לפני שלב Y)
            }
        }

        // שלב 2: בדיקת Y
        for (int i = 0; i < secret.length(); i++) {
            // אם המיקום עדיין לא G, בדוק אם האות קיימת. אם כן, Y. אם לא, משאיר את ה- '_'
            if (resultRow[i] != 'G' && containsChar(secret, guess.charAt(i))) {
                resultRow[i] = 'Y';
            }
        }
    }
